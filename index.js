...
// ==========================================
// 6. ADMIN APIs (Users, Transactions, Books, Role, Block)
// ==========================================
...
app.get('/api/admin/transactions', verifyToken, async (req, res) => {
  try {
    const db = client.db("fableDB");
    const purchaseCollection = db.collection("purchases");
    res.send(await purchaseCollection.find().toArray());
  } catch (err) { res.status(500).send({ message: err.message }); }
});

app.get('/api/admin/all-books', verifyToken, async (req, res) => {
  try {
    const db = client.db("fableDB");
    const allBooksCollection = db.collection("all_books");
    res.send(await allBooksCollection.find().toArray());
  } catch (err) { res.status(500).send({ message: err.message }); }
});

app.patch('/api/admin/update-role/:id', verifyToken, async (req, res) => {
  try {
    const db = client.db("fableDB");
    const userCollection = db.collection("user");
    const result = await userCollection.updateOne(
...
app.patch('/api/admin/toggle-block/:id', verifyToken, async (req, res) => {
  try {
    const db = client.db("fableDB");
    const userCollection = db.collection("user");
    const result = await userCollection.updateOne(
...
async function startServer() {
  try {
    ...
    // কোনো ডাটাবেস অপারেশন আগে করতে চাইলে এখানে করতে পারেন
  } catch (err) {
    console.error("❌ MongoDB Connection Failed:", err.message);
    // সার্ভার শুরু হবে, কিন্তু DB অপারেশন ব্যর্থ হবে – ভালো ডিবাগিংয়ের জন্য
  }
